...
begin
  assign(file1, 'C:\Users\xatiz\OneDrive\Рабочий стол\ЛР 15 - 16\input.txt');
  assign(file2, 'C:\Users\xatiz\OneDrive\Рабочий стол\ЛР 15 - 16\output.txt');
  reset(file1);
  rewrite(file2);
  while not eof(file1) do begin readln(file1, x); push(stek, x); end;
  if isemptystack(stek) then writeln('в файле пусто') else begin while (stek.size > 0) do writeln(file2, Pop(stek)); end;
  close(file1);
  close(file2);
  print('копирование текста завершено')
end.